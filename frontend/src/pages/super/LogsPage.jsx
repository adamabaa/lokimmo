...
export default function LogsPage() {
  ...
  return (
    <SuperAdminLayout title="Logs d'activité" subtitle="Historique de toutes les actions">
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <span style={{ fontSize: '0.8rem', color: '#8b8d96' }}>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '3rem', color: '#8b8d96', fontSize: '0.875rem' }}>
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '0.5rem', opacity: 0.3 }}>
            ...
            return (
              <div key={log.id} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
                  ...
                  <span style={{ fontSize: '0.72rem', color: '#555761', whiteSpace: 'nowrap', marginLeft: '8px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', marginBottom: '0.5rem' }}>
                  <div>
                    <div style={{ fontSize: '0.65rem', color: '#555761', textTransform: 'uppercase', marginBottom: '2px' }}>Utilisateur</div>
                    <div style={{ color: '#f0ece4', fontSize: '0.85rem', fontWeight: 500 }}>{log.user_name || '—'}</div>
                    <div style={{ color: '#555761', fontSize: '0.72rem' }}>{log.user_email || ''}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: '0.65rem', color: '#555761', textTransform: 'uppercase', marginBottom: '2px' }}>Agence</div>
                    {log.agency_name ? (
                      <>
                        <div style={{ color: '#f0ece4', fontSize: '0.85rem' }}>{log.agency_name}</div>
                        <div style={{ color: '#555761', fontSize: '0.72rem', fontFamily: 'monospace' }}>{log.agency_slug}</div>
                      </>
                    ) : (
                      <span style={{ color: '#d4a853', fontSize: '0.75rem' }}>Super Admin</span>
                    )}
                  </div>
                </div>
              ...
              <thead>
                <tr style={{ background: '#13171f', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                  {['Action', 'Utilisateur', 'Agence', 'Description', 'IP', 'Date'].map(h => (
                    <th key={h} style={{ padding: '0.75rem 1rem', textAlign: 'left', fontSize: '0.68rem', color: '#555761', textTransform: 'uppercase', letterSpacing: '0.06em', whiteSpace: 'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {logs.map((log, i) => {
                  ...
                  return (
                    <tr key={log.id}
                      style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', opacity: 0, animation: `fadeIn 0.3s ease forwards ${i * 0.03}s` }}
                      onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.02)'}
                      onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
                    >
                      <td style={{ padding: '0.75rem 1rem' }}>
                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', background: `${actionInfo.color}15`, color: actionInfo.color, borderRadius: '20px', padding: '2px 8px', fontSize: '0.72rem', fontWeight: 500, whiteSpace: 'nowrap' }}>
                          {actionInfo.icon} {actionInfo.label}
                        </span>
                      </td>
                      <td style={{ padding: '0.75rem 1rem' }}>
                      ...
                      <td style={{ padding: '0.75rem 1rem', color: '#555761', fontFamily: 'monospace', fontSize: '0.72rem' }}>{log.ip_address || '—'}</td>
                      <td style={{ padding: '0.75rem 1rem', color: '#8b8d96', whiteSpace: 'nowrap', fontSize: '0.75rem' }}>{formatDate(log.created_at)}</td>
                    </tr>
                  )
                })}
              </tbody>
      ...
      {totalPages > 1 && (
        <div style={{ display: 'flex', justifyContent: 'center', gap: '4px', marginTop: '1.25rem', flexWrap: 'wrap' }}>
          ...
          {[...Array(totalPages)].map((_, i) => (
            <button key={i + 1} onClick={() => setPage(i + 1)} style={pageBtnStyle(page === i + 1, false)}>{i + 1}</button>
          ))}
...
function StatCard({ label, value, color, icon }) {
  return (
    <div style={{ background: '#0e1219', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '12px', padding: '1rem', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ fontSize: '1.2rem', fontFamily: 'var(--font-display)', color: '#f0ece4', fontWeight: 500, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
        {value}
      </div>
    </div>
  )
