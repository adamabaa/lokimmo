...
/**
 * Service de notifications email/SMS métier
 * Tous les templates de notifications Lokimmo
 */
class NotificationMailService
{
    /**
     * Email + SMS — Loyer en retard
     */
    public static function sendLatePayment(
        ...
        $subject = "[{$agency['name']}] Rappel — Loyer en retard";

        $template = "
        <h2 style='color:#e5534b;margin:0 0 16px'>⚠️ Rappel de loyer</h2>
        ...
        <table width='100%' cellpadding='0' cellspacing='0'
            ...
            <tr>
                <td style='font-size:13px;color:#666;padding:4px 0'>Montant dû</td>
                <td align='right' style='font-size:15px;font-weight:bold;color:#e5534b'>{amount_due}</td>
            </tr>
            <tr>
                <td style='font-size:13px;color:#666;padding:4px 0'>Date d'échéance</td>
        ...
        <div style='margin-top:24px;padding:16px;background:#f5f5f5;border-radius:8px;font-size:13px;color:#777'>
            📱 Consultez votre espace locataire :<br>
    ...
    public static function sendPaymentConfirmation(
        ...
        $subject = "[{$agency['name']}] Paiement reçu — Merci";

        $template = "
        <h2 style='color:#3ecf8e;margin:0 0 16px'>✅ Paiement confirmé</h2>
        <p style='font-size:15px;color:#333;line-height:1.6'>
            Bonjour <strong>{tenant_name}</strong>,
        </p>
        <p style='font-size:15px;color:#333;line-height:1.6'>
            Nous avons bien reçu votre paiement de loyer.
        </p>
        ...
        <table width='100%' cellpadding='0' cellspacing='0'
            ...
            <tr>
                <td style='font-size:13px;color:#666;padding:4px 0'>Période</td>
                <td align='right' style='font-size:13px;color:#333'>{month}</td>
            </tr>
            <tr>
                <td style='font-size:13px;color:#666;padding:4px 0'>Montant payé</td>
        ...
        <p style='font-size:14px;color:#555;line-height:1.6'>
            Vous pouvez télécharger votre quittance depuis votre espace locataire.
        </p>
    ...
    /**
     * Email — Contrat expirant bientôt
     */
    public static function sendContractExpiring(
        ...
        $subject = "[{$agency['name']}] Votre contrat expire bientôt";

        $template = "
        <h2 style='color:{primary_color};margin:0 0 16px'>📄 Contrat arrivant à expiration</h2>
        <p style='font-size:15px;color:#333;line-height:1.6'>
            Bonjour <strong>{tenant_name}</strong>,
        </p>
        <p style='font-size:15px;color:#333;line-height:1.6'>
        ...
        <p style='font-size:14px;color:#555;line-height:1.6'>
            Veuillez contacter votre agence pour le renouveler ou organiser votre départ.
        </p>

        <div style='margin:24px 0;padding:16px;background:#fff8e6;border:1px solid #ffd080;border-radius:8px'>
            <strong style='color:#d4a853'>📞 Contactez votre agence :</strong><br>
            <span style='color:#555'>{agency_name} — {agency_phone}</span>
        </div>";
    ...
    public static function sendPortalAccess(
        ...
        $subject = "[{$agency['name']}] Votre accès au portail locataire";

        $template = "
        <h2 style='color:{primary_color};margin:0 0 16px'>🏠 Bienvenue sur votre espace locataire</h2>
        <p style='font-size:15px;color:#333;line-height:1.6'>
            Bonjour <strong>{tenant_name}</strong>,
        </p>
        <p style='font-size:15px;color:#333;line-height:1.6'>
            Votre agence <strong>{agency_name}</strong> vous a ouvert un accès
            à votre espace locataire personnel.
        </p>
    ...
    /**
     * Email — Accès portail propriétaire
     */
    public static function sendOwnerPortalAccess(
        ...
        $subject = "[{$agency['name']}] Votre accès au portail propriétaire";

        $template = "
        <h2 style='color:{primary_color};margin:0 0 16px'>🏢 Bienvenue sur votre espace propriétaire</h2>
        <p style='font-size:15px;color:#333;line-height:1.6'>
            Bonjour <strong>{owner_name}</strong>,
        </p>
        <p style='font-size:15px;color:#333;line-height:1.6'>
            <strong>{agency_name}</strong> vous a ouvert un accès à votre espace propriétaire
            où vous pouvez suivre vos biens, revenus et dépenses.
        </p>
